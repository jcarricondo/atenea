// Funciones JavaScript correspondientes al módulo de TALLER
...
// Función que carga los talleres de una sede
function cargaTalleres(id_sede){
    // Reseteamos los campos del formulario
    resetearCamposFormulario();

    // Hacemos la llamada a la función AJAX para la carga de las OPs según la sede
    var ajax = objetoAJAX();
    ajax.open("GET","../ajax/taller/taller.php?comp=cargaTalleres&id_sede=" + id_sede,"true");
    ajax.onreadystatechange=function() {
        if (ajax.readyState==4 && ajax.status==200) {
            document.getElementById("capaTalleres").innerHTML=ajax.responseText;
        }
    }
    ajax.send(null);
}
...
function cargaOPsPorSede(id_sede){
    // Cargamos primero los talleres según la sede
    cargaTalleres(id_sede);

    // Realizamos la petición al servidor para obtener la fila del buscador del taller
    var respuesta = (function () {
        var respuesta = null;
        $.ajax({
            dataType: "json",
            url: "../ajax/taller/taller.php?comp=cargaOpsPorSede",
    ...
    var uri = '<input type="text" id="input_oc" name="orden_compra" class="BuscadorInput" maxlength="50" value="" style="width:200px;" />';
...
function cargarOrdenesCompraVariasOP(id_sede){
    ...
    var ajax = objetoAJAX();
	ajax.open("GET","../ajax/taller/taller.php?comp=cargarOrdenesCompra&id_produccion=" + id_produccion + "&proveedor=" + proveedor + "&id_sede=" + id_sede,"true");
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4 && ajax.status==200) {
...
function cargaReferencia(){
    ...
    if(id_referencia != ""){
        if(id_referencia != 0){
            var ajax = objetoAJAX();
            ajax.open("GET","../ajax/taller/taller.php?comp=cargaReferencia&id_referencia=" + id_referencia + "&metodo=" + metodo + "&id_taller=" + id_taller,"true");
            ajax.onreadystatechange=function() {
                if (ajax.readyState==4 && ajax.status==200) {
...
function addReferencia(tableId,id_referencia,metodo){
	...
	if(metodo == "RECEPCIONAR"){
		table.rows[1].cells[9].innerHTML = '<input type="button" class="BotonEliminar" style="margin: 3px 0px 0px 0px;" value="RECEPCIONAR" onclick="recepcionarReferencia('+ id_referencia +','+ id_taller +')" />';
	}
	else {
		table.rows[1].cells[9].innerHTML = '<input type="button" class="BotonEliminar" style="margin: 3px 0px 0px 0px;" value="DESRECEPCIONAR" onclick="desrecepcionarReferencia('+ id_referencia +','+ id_taller +')" />';	
	}
...
// Se guardan las piezas en el STOCK según su id_taller
function recepcionarReferencia(id_referencia,id_taller){
                ...
                if(piezas >= 0) {
                    if(piezas != 0){
                        ...
                        var respuesta = (function () {
                            ...
                            $.ajax({
                                dataType: "json",
                                url: "../ajax/taller/taller.php?comp=recepcionar",
                                data: "id_referencia=" + id_referencia + "&piezas=" + piezas + "&id_taller=" + id_taller,
                                type: "GET",
                                async: false,
                                success: function (data) {
                                    respuesta = data;
                        ...
                        if(!hubo_error){
                            ...
                            table.rows[fila].cells[8].innerHTML = '<input type="button" class="BotonEliminar" style="margin: 3px 0px 0px 0px;" value="+" onclick="javascript:abrir(' + "'" + 'popup_log.php?id_albaran=' + id_albaran + '&id_referencia=' + id_referencia + '&id_taller=' + id_taller + "'" + ')" />';
                            table.rows[fila].cells[9].innerHTML = '<input type="button" class="BotonEliminar" style="margin: 3px 0px 0px 0px;" value="ELIMINAR" onclick="deshacerRecepcion(this,' + id_referencia + ','+id_taller+')" />';
...
// Función que deshace la recepción de una pieza de taller
function deshacerRecepcion(r,id_referencia,id_taller){
    ...
    var respuesta = (function () {
        ...
        $.ajax({
            dataType: "json",
            url: "../ajax/taller/taller.php?comp=deshacerRecepcion",
            data: "id_referencia=" + id_referencia + "&id_albaran=" + id_albaran + "&id_taller=" + id_taller,
            type: "GET",
            async: false,
            success: function (data) {
                respuesta = data;
...
function desrecepcionarReferencia(id_referencia,id_taller){
    // Comprobamos que no se haya añadido la referencia
	var error = comprobarIdRef(id_referencia);
                ...
                if(piezas >= 0) {
                    if(piezas != 0) {
                        ...
                        var respuesta = (function () {
                            ...
                            $.ajax({
                                dataType: "json",
                                url: "../ajax/taller/taller.php?comp=desrecepcionar",
                                data: "id_referencia=" + id_referencia + "&piezas=" + piezas + "&id_taller=" + id_taller,
                                type: "GET",
                                async: false,
                                success: function (data) {
                        ...
                        if(!hubo_error){
                            ...
                            table.rows[fila].cells[7].innerHTML = piezas;
                            table.rows[fila].cells[8].innerHTML = '<input type="button" class="BotonEliminar" style="margin: 3px 0px 0px 0px;" value="+" onclick="javascript:abrir(' + "'" + 'popup_log.php?id_albaran=' + id_albaran + '&id_referencia=' + id_referencia + '&id_taller=' + id_taller + "'" + ')" />';
                            table.rows[fila].cells[9].innerHTML = '<input type="button" class="BotonEliminar" style="margin: 3px 0px 0px 0px;" value="ELIMINAR" onclick="deshacerDesRecepcion(this,' + id_referencia + ',' + id_taller + ')" />';
...
function deshacerDesRecepcion(r,id_referencia,id_taller){
    var id_albaran = document.getElementById("id_albaran_hidden").value;
    ...
    var respuesta = (function () {
        ...
        $.ajax({
            dataType: "json",
            url: "../ajax/taller/taller.php?comp=deshacerDesRecepcion",
            data: "id_referencia=" + id_referencia + "&id_albaran=" + id_albaran + "&id_taller=" + id_taller,
            type: "GET",
            async: false,
            success: function (data) {
...
function recepcionarDesrecepcionar(id_referencia,id_produccion,id_taller){
	// Obtenemos las unidades que inserto el usuario
	var unidades_pendientes = 0;
	var unidades_entrada = document.getElementById("und_entrada-" + id_referencia + "-" + id_produccion).value;
	var mensaje = "";
	...
	if(unidades_entrada != 0){
		...
		if(confirm(mensaje)){
            ...
            var respuesta = (function () {
                ...
                $.ajax({
                    dataType: "json",
                    url: "../ajax/taller/taller.php?comp=recepcionarDesrecepcionar",
                    data: "id_referencia=" + id_referencia + "&id_produccion=" + id_produccion + "&unidades_entrada=" + unidades_entrada + "&unidades_pendientes=" + unidades_pendientes + "&id_taller=" + id_taller + "&id_usuario=" + id_usuario,
                    type: "GET",
                    async: false,
                    success: function (data) {
                        respuesta = data;
...
function cargaMotivos(id_taller,tipo_albaran){
    // Realizamos la petición al servidor para obtener los motivos del albaran
    var respuesta = (function () {
        var respuesta = null;
        $.ajax({
            dataType: "json",
            url: "../ajax/taller/taller.php?comp=cargaMotivos",
            data: "id_taller=" + id_taller + "&tipo_albaran=" + tipo_albaran,
            type: "GET",
            async: false,
            success: function (data) {
                respuesta = data;
...
function cargaMotivosBuscador(id_sede){
    ...
    var respuesta = (function () {
        ...
        $.ajax({
            dataType: "json",
            url: "../ajax/taller/taller.php?comp=cargaMotivosBuscador",
            data: "id_sede=" + id_sede,
            type: "GET",
            async: false,
            success: function (data) {
...
function cerrarAlbaran(id_albaran){
	if(confirm("¿Desea finalizar la operación?")){
		...
		if(num_filas == 0){
			// Albaran vacio
			window.location="../taller/albaranes.php?cerrarAlbaran=1&id_albaran=" + id_albaran + "&vacio=1";	
		}
		else{
			window.location="../taller/albaranes.php?cerrarAlbaran=1&id_albaran=" + id_albaran + "&vacio=0";	
		}
	}
}
...
// Función para resetear los campos de los formularios de Taller
