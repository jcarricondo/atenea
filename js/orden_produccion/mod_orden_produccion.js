...
function MostrarCabProduccion(){
	...
	var cab_sel_produccion = document.getElementsByName("cab_sel_produccion[]");
	...
	for(i=-1;i<id_cab_produccion.length;i++){
		...
		if (i==-1) {
			...
			for (j=0;j<cab_sel_produccion.length;j++){
				if (id_cab_produccion[i].value == cab_sel_produccion[j].value){
					opcion_cabina.selected = "selected";	
				}
			}
...
// Función que muestra en el select todas las cabinas creadas	
function MostrarTodasCabinas(){
	...
	var cab_sel_todas = document.getElementsByName("cab_sel_todas[]");
	// Insertamos el select y vamos añadiendo cada una de las cabinas
	lista_cabinas.appendChild(selectCabinas);
	for(i=-1;i<id_todas_cabinas.length;i++){
		...
		if (i==-1) {
			...
			for (j=0;j<cab_sel_todas.length;j++){
				if (id_todas_cabinas[i].value == cab_sel_todas[j].value){
					opcion_cabina.selected = "selected";	
				}
			}
...
// Función que muestra todos los perifericos creados en el select de modificación 	
