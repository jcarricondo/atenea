...
$total_refs_act_toro_no_smk = 0;
$total_refs_act_bra_no_smk = 0;
...
for($i=0;$i<count($res);$i++){
	...
	if($activo_smk == 0){
		...
		if($i % 2 == 0) $color = ' background-color: #fff; color: black;';
	...
	<tr>
		...
		<td style="'.$color.'" align="center">'.$repetidas.'</td>
	</tr>';
	...
	if($res_toro != NULL) {
		...
		// Comprobamos si existe en TORO pero no existe en SMK 
		// if($activo_smk != 0){
			...
			if(($nombre == $nombre_toro) && ($part_proveedor_referencia == $part_proveedor_referencia_toro)){
				...
				if($unidades == $unidades_toro){
					if($pack_precio == $pack_precio_toro){
						...
						if(!$es_tornilleria){
							...
							$diferencia_toro = '<span style="color: green;">LA REFERENCIA FUE ACTUALIZADA</span>'; 	
							$total_refs_ok_toro++;
							$existe_ref = true;
						}					
					}
				...
				if($res_id_ref != NULL){
					...
					$diferencia_toro .= '. ';
					// Comprobamos tambien si el pack_precio es diferente
					if($pack_precio != $pack_precio_toro){
						if(!$es_tornilleria){
							$diferencia_toro .= '<span style="color: orange;">PACK_PRECIO DISTINTO EN TORO</span>'; 	
							$total_refs_pack_precio_toro++;
		...
		/* }
			...
			$diferencia_toro = '<span style="color: #FF00BF;">LAS REF ESTA ACTIVA EN TORO PERO NO EN SMK</span>'; 
			$total_refs_act_toro_no_smk++;
			$existe_ref = true;
		}
		*/
	}
	...
	if($res_bra != NULL) {
		...
		// Comprobamos si existe en BRA pero no existe en SMK 
		// if($activo_smk != 0){	
			// Comprobamos la referencia del proveedor y el nombre referencia
				...
				if($unidades == $unidades_bra){
					if($pack_precio == $pack_precio_bra){
						...
						if(!$es_tornilleria){
							...
							$diferencia_bra = '<span style="color: green;">LA REFERENCIA FUE ACTUALIZADA</span>'; 	
							$total_refs_ok_bra++;
							$existe_ref = true;
						}
					}
		...
		/* }
			...
			$diferencia_bra = '<span style="color: #FF00BF;">LAS REF ESTA ACTIVA EN BRASIL PERO NO EN SMK</span>'; 
			$total_refs_act_bra_no_smk++;
			$existe_ref = true;
		}	
		*/
	}
...
/*
<tr>
	<td style="color: #FF00BF;">TOTAL REFS. NO EXISTEN EN SMK. SI EN TORO: '.$total_refs_act_toro_no_smk.'</td>
</tr>
<tr>
	<td style="color: #FF00BF;">TOTAL REFS. NO EXISTEN EN SMK. SI EN BRASIL: '.$total_refs_act_bra_no_smk.'</td>
</tr>			
*/

header("Content-type: application/vnd.ms-excel");
header("Content-Disposition: attachment; filename=DiferenciasRefs.xls");
