...
// En este script se importan los albaranes de piezas en los talleres
...
for($i=0;$i<count($res_albaranes);$i++) {
    ...
    $insertAlbaranes = sprintf("insert into talleres_albaranes (nombre_albaran,tipo_albaran,id_participante,id_tipo_participante,
                                  motivo,id_usuario,id_taller,fecha_creado,activo) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)",
    ...
    if($db->ejecutarSoloConsulta()){
        ...
        for($j=0;$j<count($res_referencias);$j++){
            ...
            $insertReferencias = sprintf("insert into talleres_albaranes_referencias (id_albaran,id_referencia,nombre_referencia,nombre_proveedor,
        ...
        for($j=0;$j<count($res_movimientos);$j++){
            ...
            $insertMovimientos = sprintf("insert into talleres_albaranes_log (id_albaran,id_referencia,id_produccion,piezas,metodo,fecha_creado,activo)
