...
for($i=0;$i<count($res_duplicadas);$i++){
	...
	$refAlmacenSql = sprintf("select * from talleres_albaranes_log where activo=1 and id_referencia=%s",
		$db->makeValue($id_referencia_duplicada,"int"));
	$db->setConsulta($refAlmacenSql);
	$db->ejecutarConsulta();
...
for($i=0;$i<count($res_duplicadas);$i++){
	...
	$refAlmacenSql = sprintf("select * from talleres_albaranes_referencias where activo=1 and id_referencia=%s",
		$db->makeValue($id_referencia_duplicada,"int"));
	$db->setConsulta($refAlmacenSql);
	$db->ejecutarConsulta();
