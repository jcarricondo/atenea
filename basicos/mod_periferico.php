...
// Este fichero modifica un periferico de basicos
...
if(isset($_POST["guardandoPeriferico"]) && $_POST["guardandoPeriferico"] == 1) {
	...
	$duplicado = $_POST["duplicar_periferico"];
	...
	if(($nombre == '') || ($referencia == '') || ($descripcion == '') || ($version == '')){
		...
		if($seguir){
			...
			$resultado = $perifericos->compruebaTablaArchivos($id_componente,$archivos_tabla);
			if($resultado == 1){
				if(isset($_FILES["archivos"])) {
					...
					while(($i<count($_FILES["archivos"]["name"])) and ($error_archivo == 0)) {
						if($_FILES["archivos"]["name"][$i] != NULL) {
							...
							$nombre_archivo[$i] = $num_ramdom.'_'.basename($_FILES['archivos']['name'][$i]);
				...
				$perifericos->datosNuevoPeriferico($id_componente,$nombre,$referencia,$descripcion,$version,$referencias,$piezas,2,$nombre_archivo,$interfaces,$estado,$prototipo,$kits);
				...
				if($resultado == 1) {
					...
					if($metodo == "masivo"){
						...
						$proceso = "MODIFICACION PERIFERICO";
						include("../basicos/log_importacion_referencias_componente.php");
					}

					// Si el usuario ha pulsado Duplicar Periferico
					if($duplicado == 1) {
						...
						//Obtenemos los nombres de los archivos del periferico original
						$perifericos->dameNombres_archivos($id_componente);
						$nombres = $perifericos->nombres_archivos;
						...
						for($i=0;$i<count($nombres);$i++) {
							...
							// Guarda en el array nombre_archivo los nombres de los archivos sin el numero random del los archivos del periferico original
							$nombre_archivo[$i] = $aux[$i][1];
							// Le metemos un nuevo random para que no sea igual que el del periferico original
						...
						$perifericos->datosNuevoPeriferico(NULL,$nombre,$referencia,$descripcion,$version,$referencias,$piezas,2,$nombre_archivo,$interfaces,$estado,$prototipo,$kits);
						$resultado = $perifericos->guardarCambios();
						if($resultado == 1) {
							header("Location: perifericos.php?per=duplicado");
						}
						else {
							$mensaje_error = $perifericos->getErrorMessage($resultado);
						}
					}
					// Duplica el periferico y eleva la version al entero mas proximo
						...
						$resultado = $perifericos->guardarCambios();
						if($resultado == 1) {
							header("Location: perifericos.php?per=actualizado");
						}
						else {
							$mensaje_error = $perifericos->getErrorMessage($resultado);
						}
						...
						header("Location: perifericos.php?per=modificado");
					}
				}
				else {
					$mensaje_error = $perifericos->getErrorMessage($resultado);
...
// Titulo de pagina
$titulo_pagina = "Básico > Modifica periferico";
$pagina = "mod_periferico";
include ('../includes/header.php');
$ref_perifericos->setValores($_GET["id"]);
...
<div id="ContenedorCentral">
    ...
    <h3> Modificación de periferico </h3>
    <form id="FormularioCreacionBasico" name="modificarPeriferico" onsubmit="return SeleccionarComponentes()" action="mod_periferico.php?id=<?php echo $perifericos->id_componente; ?>" method="post" enctype="multipart/form-data">
    	<br />
           		...
           		<div class="CajaReferencias">
            		<div id="CapaTablaIframe">
                        ...
                        <?php
							...
							// alguna referencia del periferico, no se veia reflejado el cambio.
        ...
        <br/>
 		<div class="ContenedorCamposCreacionBasico">
           	<div class="LabelCreacionBasico">Mostrar archivos del periferico</div>
          		<div class="CajaReferencias">
                    <div id="CapaTablaIframe">
        				...
        				<tr>
 						...
 						<?php
							$perifericos->dameId_archivo($perifericos->id_componente);
							$ids_archivos = $perifericos->ids_archivos;

							for($i=0;$i<count($ids_archivos);$i++) {
								$array_ids_archivos[] = $ids_archivos[$i]["id_archivo"];
								$perifericos->cargaDatosArchivosPerifericoId($array_ids_archivos[$i]);
						?>
						<tr><td><?php echo $perifericos->nombre_archivo;?><input type="hidden" name="archivos_tabla[]" id="archivos_tabla[]" value="<?php echo $perifericos->nombre_archivo;?>" /></td><td style="text-align:center"><?php echo $perifericos->fecha_subida;?></td><td style="text-align:center"><input type="button" id="descargar" name="descargar" class="BotonEliminar"  value="DESCARGAR" onclick="window.open('download.php?id=<?php echo $perifericos->nombre_archivo;?>')"/> </td><?php if($modificar) { ?><td style="text-align:center"><input type="checkbox" name="chkboxArch" value="<?php echo $perifericos->id_archivo;?>" /></td><?php } ?></tr>
                    <?php
					}
					?>
                    </table>
    	...
    	<br />
    	<?php if($modificar) { ?>
        	<input type="checkbox" id="duplicar_periferico" name="duplicar_periferico" value="1" /> <div class="label_check_precios">Duplicar periferico</div>
        <?php } ?>
        <br/>
        <br/>
        <br/>
