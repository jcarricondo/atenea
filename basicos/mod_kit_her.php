...
<div id="ContenedorCentral">
    ...
    <form id="FormularioCreacionBasico" name="modificarKit" action="mod_kit_her.php?id=<?php echo $kt->id_componente; ?>" method="post" enctype="multipart/form-data">
    	<br />
            	...
            	<div id="CapaTablaIframe">
                    ...
                    <?php
						...
						for ($i=0; $i<count($resultadosBusqueda); $i++) {
							...
							$tiene_padres = $ref_heredada->tienePadres($ref_modificada->id_referencia);
							if(!$tiene_padres) {?>
								<tr>
									...
									<?php
										if ($modificar) { ?>
											<td style="text-align:center">
												<input type="text" name="piezas[]" id="piezas[]" class="CampoPiezasInput" value="<?php echo number_format($ref_componente->piezas, 2, '.', ''); ?>" onblur="validarHayCaracter(<?php echo $fila; ?>)"/>
											</td>
									<?php
										} else { ?>
        ...
        <div class="ContenedorCamposCreacionBasico">
           	<div class="LabelCreacionBasico">Mostrar archivos del kit</div>
			    ...
			    <?php if($modificar){ ?>
                	<input type="button" id="quitar" name="quitar" class="BotonQuitar" value="QUITAR" onclick="removeRowArchivos(mitablaArchivos)"  />
                <?php } ?>
        </div>
        <br/>
        <?php if($modificar){ ?>
