...
$dir_documentacion_plantilla = str_replace("/", "_",$dir_documentacion.$barra_directorio.$nombre_plantilla."_v".$version_plantilla);
...
if(!empty($res_perifericos)) {
    for($i=0;$i<count($res_perifericos);$i++) {
        ...
        if($periferico_no_vacio){
            ...
            $dir_periferico_actual = str_replace("/", "_",$dir_perifericos.$barra_directorio.$nombre_periferico."_v".$version_periferico);
...
$filename = $nombre_plantilla."_v".$version_plantilla.".zip";
$zip = new PclZip($filename);
$zip->create($nombre_plantilla."_v".$version_plantilla);
