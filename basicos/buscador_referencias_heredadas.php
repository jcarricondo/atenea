<?php
// Este fichero realiza la busqueda de referencias. Es el buscador de referencias heredadas de basicos
...
function add_referencia(id_referencia){
	...
	if (!error_piezas) {
		...
		boton = '<input type="button" id="menos" name="menos" value="-" onclick="javascript:removeRowHeredada()"  />'
		...
		window.opener.addRowHeredada('mitablaHeredadas',id_referencia);
	}
...
<div id="ContenedorCentralReferencias">
	<h3> Añadir referencia heredable </h3>
	<div id="ContenedorBuscadorReferencias">
		<h4> Buscar la referencia para añadir </h4>
   		<form name="BuscadorReferencias" id="BuscadorReferencias" action="buscador_referencias_heredadas.php?id_ref=<?php echo $id_referencia;?>" method="post">
        	...
        	<?php
				...
				for($i=0;$i<count($resultadosBusqueda);$i++) {
					...
					<tr>
                        ...
                        <td style="text-align:center">
                            <form name="BuscadorReferenciaHeredada" id="BuscadorReferenciaHeredada" action="#" method="post">
								<input type="button" onclick="javascript:add_referencia(<?php echo $ref->id_referencia;?>);" value="+" />
                                <input type="hidden" id="guardandoReferenciaHeredada" name="guardandoReferenciaHeredada" />
                            </form>
                        </td>                        
					</tr> 
               		<?php
