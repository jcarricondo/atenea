...
// Este fichero modifica una interfaz de basicos
...
if(isset($_POST["guardandoInterfaz"]) and $_POST["guardandoInterfaz"] == 1) {
	...
	$duplicado = $_POST["duplicar_interfaz"];
	...
	if(($nombre == '') or ($referencia == '') or ($descripcion == '') or ($version == '')){
		...
		if($seguir){
			...
			$resultado = $interfaces->compruebaTablaArchivos($id_componente,$archivos_tabla);
			if($resultado == 1){
				...
				$interfaces->datosNuevoInterfaz($id_componente,$nombre,$referencia,$descripcion,$version,$referencias,$piezas,4,$nombre_archivo,$estado,$prototipo);
				$resultado = $interfaces->guardarCambios();
				if($resultado == 1) {
					// Guardamos un log en caso de importacion masiva
					if($metodo == "masivo"){
						$id_usuario = $_SESSION["AT_id_usuario"];
						$tipo_componente = "INTERFAZ";
						$proceso = "MODIFICACION INTERFAZ";
						include("../basicos/log_importacion_referencias_componente.php");
					}

					// Si el usuario ha pulsado Duplicar Interfaz
					if($duplicado == 1) {
						...
						//Obtenemos los nombres de los archivos de la interfaz original
						$interfaces->dameNombres_archivos($id_componente);
						$nombres = $interfaces->nombres_archivos;
						...
						for ($i=0;$i<count($nombres);$i++) {
							...
							// Guarda en el array nombre_archivo los nombres de los archivos sin el numero random del los archivos de la interfaz original
							$nombre_archivo[$i] = $aux[$i][1];
							// Le metemos un nuevo random para que no sea igual que el de la interfaz original
						...
						$interfaces->datosNuevoInterfaz(NULL,$nombre,$referencia,$descripcion,$version,$referencias,$piezas,4,$nombre_archivo,$estado,$prototipo);
						$resultado = $interfaces->guardarCambios();
						if($resultado == 1) {
							header("Location: interfaces.php?interface=duplicado");
						}
						else {
							$mensaje_error = $interfaces->getErrorMessage($resultado);
						}
					}
					// Duplica la interfaz y eleva la version al entero mas proximo
						...
						$resultado = $interfaces->guardarCambios();
						if($resultado == 1) {
							header("Location: interfaces.php?interface=actualizado");
						}
						else {
							$mensaje_error = $interfaces->getErrorMessage($resultado);
						}
						...
						header("Location: interfaces.php?interface=modificado");
					}
				}
				else {
					$mensaje_error = $interfaces->getErrorMessage($resultado);
...
$estado = $interfaces->estado;
$prototipo = $interfaces->prototipo;
$referencias = $interfaces->referencias;

// Titulo de pagina
$titulo_pagina = "Básico > Modifica interfaz";
$pagina = "mod_interfaz";
include ('../includes/header.php');
echo '<script type="text/javascript" src="../js/basicos/mod_interfaz.js"></script>';
echo '<script type="text/javascript" src="../js/basicos/mod_interfaz_adjuntos.js"></script>';
$ref_interfaces->setValores($_GET["id"]);
$ref_interfaces->realizarConsulta();
$resultadosBusqueda = $ref_interfaces->referencias_componentes;
$componente = "interfaz";
?>
...
<div id="ContenedorCentral">
    ...
    <h3> Modificacion de interfaz </h3>
    <form id="FormularioCreacionBasico" name="modificarInterfaz" action="mod_interfaz.php?id=<?php echo $interfaces->id_componente; ?>" method="post" enctype="multipart/form-data">
    	<br />
            		...
            		<div id="CapaTablaIframe">
                        ...
                        <tr>
                        	...
                        	<td id="enlaceComposites">
                        		<a href="mod_referencia.php?id=<?php echo $ref_componente->id_referencia;?>"/>
                        			<?php	
        ...
        <div id="coste_componente" class="ContenedorCamposCreacionBasico">
           	<div class="LabelCreacionBasico">Coste Interfaz</div>
        ...
        <br/>
        <div class="ContenedorCamposCreacionBasico">
           	<div class="LabelCreacionBasico">Mostrar archivos de la interfaz</div>
          		<div class="CajaReferencias">
                    <div id="CapaTablaIframe">
        				...
        				<tr>
 						...
 						<?php
							$interfaces->dameId_archivo($interfaces->id_componente);
							$ids_archivos = $interfaces->ids_archivos;

							for($i=0;$i<count($ids_archivos);$i++) {
								$array_ids_archivos[] = $ids_archivos[$i]["id_archivo"];
								$interfaces->cargaDatosArchivosInterfazId($array_ids_archivos[$i]);
						?>
						<tr><td><?php echo $interfaces->nombre_archivo;?><input type="hidden" name="archivos_tabla[]" id="archivos_tabla[]" value="<?php echo $interfaces->nombre_archivo;?>" /></td><td style="text-align:center"><?php echo $interfaces->fecha_subida;?></td><td style="text-align:center"><input type="button" id="descargar" name="descargar" class="BotonEliminar"  value="DESCARGAR" onclick="window.open('download.php?id=<?php echo $interfaces->nombre_archivo;?>')"/> </td><?php if($modificar) { ?><td style="text-align:center"><input type="checkbox" name="chkboxArch" value="<?php echo $interfaces->id_archivo;?>" /></td><?php } ?></tr>
                    <?php
					}
					?>
                    </table>
        ...
        <br/>
        <?php if($modificar) { ?>
        	<input type="checkbox" id="duplicar_interfaz" name="duplicar_interfaz" value="1" /> <div class="label_check_precios">Duplicar interfaz</div>
        <?php } ?>
        <br/>
        <br/>
        <br/>
        <div class="ContenedorBotonCreacionBasico">
            ...
            <?php if($modificar) { ?>
            	<input type="hidden" id="guardandoInterfaz" name="guardandoInterfaz" value="1"/>
