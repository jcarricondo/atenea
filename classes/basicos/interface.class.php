...
class Interfaz extends MySQL {
	...
	function cargarDatos($id_componente,$interfaz,$referencia,$version,$descripcion,$id_tipo,$estado,$prototipo) {
	...
	function cargaDatosInterfazId($id_componente) {
	...
	function cargarDatosArchivosInterfaz($id_archivo,$id_tipo,$id_componente,$nombre_archivo,$fecha_subida) {
	...
	function cargaDatosArchivosInterfazId($id) {
		$consultaSql = sprintf("select * from componentes_archivos where id_archivo=%s ",
			$this->makeValue($id, "int"));
		$this->setConsulta($consultaSql);
		$this->ejecutarConsulta();
		$resultados = $this->getPrimerResultado();
		$this->cargarDatosArchivosInterfaz(
	...
	// Se hace la carga de datos  de una nueva interfaz
	function datosNuevoInterfaz($id_componente = NULL,$nombre,$referencia,$descripcion,$version,$referencias,$piezas,$id_tipo = 4,$nombre_archivo,$estado,$prototipo) {
		$this->id_componente = $id_componente;
		$this->nombre = $nombre;
		$this->referencia = $referencia;
		$this->descripcion = $descripcion;
	...
	// Guarda los cambios realizados en la interfaz
	function guardarCambios() {
		// Si el id_componente es NULL lo toma como una nueva interfaz
		if($this->id_componente == NULL) {
			// Comprueba si hay otra interfaz con el mismo nombre
			if(!$this->comprobarInterfazDuplicada()) {
				...
				if($this->ejecutarSoloConsulta()) {
					...
					if($this->id_componente != NULL){
						...
						while($i<count($this->nombre_archivo) and (!$fallo) ) {
							$consulta_archivos = sprintf("insert into componentes_archivos (id_tipo, id_componente, nombre_archivo, fecha_subida, activo) value (4,%s,%s,current_timestamp,1)",
    							$this->makeValue($this->id_componente, "int"),
			...
			if(!$this->comprobarInterfazDuplicada()) {
				...
				if($this->ejecutarSoloConsulta()) {
					...
					if(!empty($this->nombre_archivo)) {
						...
						$fallo = false;
						while($i<count($this->nombre_archivo) and (!$fallo) ) {
							$consulta_archivos = sprintf("insert into componentes_archivos (id_tipo, id_componente, nombre_archivo, fecha_subida, activo) value (1,%s,%s,current_timestamp,1)",
    							$this->makeValue($this->id_componente, "int"),
    							$this->makeValue($this->nombre_archivo[$i], "text"));
							$this->setConsulta($consulta_archivos);
							if ($this->ejecutarSoloConsulta()) {
	...
	// Comprueba si hay otra interfaz con el mismo nombre
	// Devuelve true si hay interfaces duplicadas
	function comprobarInterfazDuplicada() {
		if($this->id_componente == NULL) {
			$consulta = sprintf("select id_componente from componentes where nombre=%s and version=%s and activo=1 and id_tipo=4",
				$this->makeValue($this->nombre, "text"),
				$this->makeValue($this->version, "float"));
		}
		else {
			$consulta = sprintf("select id_componente from componentes where nombre=%s and version=%s and activo=1 and id_tipo=4 and id_componente<>%s",
				$this->makeValue($this->nombre, "text"),
				$this->makeValue($this->version, "float"),
				$this->makeValue($this->id_componente, "int"));
		}
	...
	function eliminar(){
		//ELIMINAR
		$consulta = sprintf("update componentes set componentes.activo=0 where id_tipo=4 and id_componente=%s",
			$this->makeValue($this->id_componente, "int"));
			$this->setConsulta($consulta);
		if($this->ejecutarSoloConsulta()) {
			return 6;
	...
	function eliminarReferenciasInterfaz(){
		// Eliminar todas las referencias de componentes_referencias asociadas a ese componente.
		$consulta_refs_borrar = sprintf ("select count(id) as Total from componentes_referencias where componentes_referencias.id_componente =%s",
			$this->makeValue($this->id_componente, "int"));
		$this->setConsulta($consulta_refs_borrar);
	...
	// Funcion que elimina los archivos de una interfaz cuando se quitan en la modificacion
	...
	// Funcion que elimina los archivos de una interfaz cuando se elimina la interfaz
	function quitarArchivoInterfaz($id_componente) {
	...
	// Funcion para obtener los ids de las interfaces del componente id_componente
	function dameIdsInterfaces($id_componente) {
		$consultaId = sprintf("select componentes_interfaces.id_interfaz from componentes_interfaces where componentes_interfaces.id_componente=%s and activo=1 ",
			$this->makeValue($id_componente, "int"));
	...
	// Funcion para obtener los componentes que tienen la id_interfaz
	function dameComponentesConInterfaz($id_interfaz){
		$consulta = sprintf("select id_componente, id_tipo_componente from componentes_interfaces where id_interfaz=%s and activo=1 and id_componente in
                                (select id_componente from componentes where activo=1) order by id_componente",
			$this->makeValue($id_interfaz, "int"));
		$this->setConsulta($consulta);
		$this->ejecutarConsulta();
	...
	function getErrorMessage($error_num) {
		switch($error_num) {
			case 2:
				return 'Ya existe una interfaz con ese nombre y esa versión en la base de datos<br/>';
			break;
			case 3:
				return 'Se produjo un error al guardar la nueva interfaz<br/>';
			break;
			case 4:
				return 'Se produjo un error al modificar los datos de la interfaz<br/>';
			break;
			case 5;
				return 'Se produjo un error al eliminar la interfaz<br/>';
			break;
			case 8;
				return 'Se produjo un error al eliminar las referencias de la interfaz<br/>';
			break;
			case 9;
				return 'Se produjo un error al insertar los archivos de mecanica de la interfaz<br/>';
			break;
			case 10:
				return 'Se produjo un error al desactivar un archivo adjunto de la interfaz<br/>';
			break;
		}
	}
}
