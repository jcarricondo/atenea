...
class Referencia_Componente extends MySQL {
	...
	// Devuelve los ids de las interfaces que pertenecen a un componente
	...
	// Comprueba si el id_Componente es una interfaz
	function esIdComponenteInterfaz($id_componente){
		...
		if($this->getNumeroFilas() != 0) {
			...
			if ($this->resultados[0]["id_tipo"] == 4) {
				return true;	
			}
			else { 
				return false;
	...
	// Recorre las referencias de la interfaz y añade las referencias al componente
	function addReferenciasInterfazAlComponente($referencias_interfaz,$referencias_componente) {
		unset($referencias_aux);
		$referencias_aux = $referencias_componente;
		for($i=0;$i<count($referencias_interfaz);$i++){
			$id_referencia = $referencias_interfaz[$i]["id_referencia"];
			$piezas = $referencias_interfaz[$i]["piezas"];
			$encontrado = false;
			$j=0;
			...
			if (!$encontrado){
				...
				if($referencias_aux != NULL){ 
					array_push($referencias_aux,$referencias_interfaz[$i]);
				}
				else {
					$referencias_aux[] = $referencias_interfaz[$i];
				}
			}
			// Modificamos el array de referencias del componente por el array modificado con las referencias de la interfaz
