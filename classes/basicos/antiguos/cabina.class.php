...
class Cabina extends MySQL {
	...
	function cargaDatosCabinaId($id_componente) {
	...
	function cargarDatosArchivosCabina($id_archivo,$id_tipo,$id_componente,$nombre_archivo,$fecha_subida) {
			...
			if(!$this->comprobarCabinaDuplicada()) {
				...
				if($this->ejecutarSoloConsulta()) {
					...
					if($this->id_componente != NULL){
						...
						// Insertamos los kits que tenga asociado la cabina
						for($i=0;$i<count($this->kits);$i++){
							$consulta = sprintf("insert into componentes_kits(id_tipo_componente,id_componente,id_kit,fecha_creado,activo) value(1,%s,%s,current_timestamp,1)",
								$this->makeValue($this->id_componente, "int"),
	...
	// Comprueba si hay otra cabina con el mismo nombre
	// Devuelve true si hay cabinas duplicadas
	function comprobarCabinaDuplicada() {
		if($this->id_componente == NULL) {
	...
	function eliminar(){
		//ELIMINAR
		$consulta = sprintf("update componentes set componentes.activo=0 where id_tipo=1 and id_componente=%s",
			$this->makeValue($this->id_componente, "int"));
			$this->setConsulta($consulta);
		if($this->ejecutarSoloConsulta()) {
			return 6;
	...
	function eliminarReferenciasCabina(){
		// Eliminar todas las referencias de componentes_referencias asociadas a ese componente.
		$consulta_refs_borrar = sprintf ("select count(id) as Total from componentes_referencias where componentes_referencias.id_componente =%s",
			$this->makeValue($this->id_componente, "int"));
		$this->setConsulta($consulta_refs_borrar);
	...
	// Funcion que elimina los archivos de una cabina cuando se quitan en la modificacion
	...
	// Funcion que elimina los archivos de una cabina cuando se elimina la cabina
	function quitarArchivoCabina($id_componente) {
	...
	function getErrorMessage($error_num) {
		switch($error_num) {
			case 2:
				return 'Ya existe una cabina con ese nombre y esa versión en la base de datos<br/>';
			break;
			case 3:
