<?php
// Ficha del simulador del almacen con la posibilidad de cambiar su estado
...
if(empty($id_almacen_usuario)) $id_almacen_usuario;
...
if(isset($_POST["guardandoSimulador"]) and $_POST["guardandoSimulador"] == 1) {
    $id_simulador = $_POST["id_simulador"];
    $comentarios = $_POST["comentarios"];

    // Guardamos los comentarios
    $resultado = $simuladorAlmacen->guardarComentarios($id_simulador,$comentarios);
    if($resultado == 1){
        // Redirigimos al listado de Simuladores
        header("Location: listado_simuladores.php?realizandoBusqueda=1&id_simulador=".$id_simulador);
    }
    else {
...
$fecha_creacion = $simuladorAlmacen->fecha_creado;
...
// Admin Global con selección de simulador brasileño
...
$titulo_pagina = "Almacen Simulador > Ficha del Simulador";
$pagina = "ficha_simulador";
...
<div id="ContenedorCentral">
    ...
    <h3>Ficha del Simulador</h3>

    <form id="FormularioCreacionBasico" name="fichaSimulador" action="ficha_simulador.php" method="post">
        <br/>
        ...
        <div class="ContenedorCamposCreacionBasico">
            ...
            <label id="nombre_almacen_simulador" class="LabelInfoOP" style="width:750px;"><?php echo $nombre_almacen;?></label>
        </div>
        <div class="ContenedorCamposCreacionBasico">
        ...
        <div class="ContenedorCamposCreacionBasico">
            ...
            <?php
                // Comprobamos que el usuario pertenece al mismo almacen donde esta el simulador
                if($puedeModificar){ ?>
        ...
        <br/>
        <?php
            // Comprobamos que el usuario pertenece al mismo almacen donde está el simulador
            if($puedeModificar){ ?>
                <div id="cambiar_estado" class="ContenedorCamposCreacionBasico">
                    ...
                    <div id="boton_estado">
                        <?php
                            if($estado == "AVERIADO"){ ?>
                                <input type="button" class="BotonEliminar" style="margin-left: 10px;" value="EN REPARACION" onclick="cambiarEstado(<?php echo $id_simulador;?>,'<?php echo $estado;?>')">
                        <?php
                            }
                            else if ($estado == "EN REPARACION"){ ?>
                                <input type="button" class="BotonEliminar" style="margin-left: 10px;" value="OPERATIVO" onclick="cambiarEstado(<?php echo $id_simulador;?>,'<?php echo $estado;?>')">
                        <?php
                            }
                            else { ?>
                                <input type="button" class="BotonEliminar" style="margin-left: 10px;" value="AVERIADO" onclick="cambiarEstado(<?php echo $id_simulador;?>,'<?php echo $estado;?>')">
                        <?php
                            }
                        ?>
                    </div>
        ...
        <?php
            if($puedeModificar){ ?>
                ...
                <input type="hidden" id="guardandoSimulador" name="guardandoSimulador" value="1"/>
                <input type="hidden" id="id_simulador" name="id_simulador" value="<?php echo $id_simulador; ?>"/>
        <?php
            }
            else{ ?>
                <input type="button" class="BotonEliminar" style="margin-left: 5px;" value="VOLVER" onclick="history.back();" />
