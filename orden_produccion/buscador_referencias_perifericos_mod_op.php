<?php 
// Este es el fichero del buscador de referencias de perifericos en la seccion de modificacion de una orden de produccion
include_once("../classes/mysql.class.php");
include_once("../classes/orden_produccion/incluir_referencia_libre.class.php");
include_once("../classes/orden_produccion/listado_incluir_referencia_libre.class.php");
...
{
	referencia = document.getElementById("referencia_per-" + id_referencia).value;
	proveedor = document.getElementById("proveedor_per-" + id_referencia).value;
	ref_proveedor = document.getElementById("ref_proveedor_per-" + id_referencia).value;
	nombre_pieza = document.getElementById("nombre_pieza_per-" + id_referencia).value;
	cantidad = document.getElementById("cantidad_per-" + id_referencia).value;
	piezas = document.getElementById("piezas_per-" + id_referencia).value;
	periferico = document.getElementById("periferico_n").value;
	...
	if (!error_piezas) {
		...
		fin_enlace = '</a>';
		cadena_identificador = '<input type="hidden" name="REFS_PER_' + periferico + '[] id="REFS_PER_' + periferico + '[] value="' + id_referencia + '" />';
		cadena_unidades_paq =  '<input type="hidden" name="UDS_PERS_' + periferico + '[] id="UDS_PERS_' + periferico + '[] value="' + cantidad + '"/>';
		identificador = id_referencia;
		boton = '<input type="button" id="menos" name="menos" value="-" onclick="javascript:removeRowPer()"  />'
		...
		window.opener.ref = enlace + referencia + fin_enlace + cadena_identificador + cadena_unidades_paq;
		window.opener.prov = proveedor;
		window.opener.ref_prov = ref_proveedor;
		window.opener.nom_pieza = nombre_pieza;
		window.opener.cant = cantidad /*+ cadena_unidades_paq*/;
		...
		window.opener.perif = periferico;
	
		window.opener.addRowPeriferico('mitabla_' + periferico,id_referencia,periferico);
		//window.close();
	}
...
<div id="ContenedorCentralReferencias">
	<h3> Añadir referencia para la Orden de Producción </h3>
		...
		?>
        	...
        	<?php
				...
				for($i=0;$i<count($resultadosBusqueda);$i++) {
					...
					$ref_per = new Referencia_Libre();
					$datoRef = $resultadosBusqueda[$i];
					$ref_per->cargaDatosReferenciaLibreId($datoRef["id_referencia"]);
					?>
					<tr>
						<td style="text-align: center;">
							<?php echo $ref_per->id_referencia; ?>	
						</td>
						<td>
							<?php 
								if (strlen($ref_per->referencia) > $max_caracteres){
									echo substr($ref_per->referencia,0,50).'...';
								}
								else {
									echo $ref_per->referencia;	
								}
							?>
                            <input type="hidden" name="referencia_per-<?php echo $ref_per->id_referencia;?>" id="referencia_per-<?php echo $ref_per->id_referencia;?>" value="<?php echo $ref_per->referencia;?>" />
						</td>
						<td>
							<?php echo $ref_per->proveedor; ?>
                            <input type="hidden" name="proveedor_per-<?php echo $ref_per->id_referencia;?>" id="proveedor_per-<?php echo $ref_per->id_referencia;?>" value="<?php echo $ref_per->proveedor;?>" />
						</td>
                        <td>
                        	<?php $ref_per->vincularReferenciaProveedor();?>
                            <input type="hidden" name="ref_proveedor_per-<?php echo $ref_per->id_referencia;?>" id="ref_proveedor_per-<?php echo $ref_per->id_referencia;?>" value="<?php echo $ref_per->ref_proveedor;?>" />
                        </td>
						<td>
							<?php 
								if (strlen($ref_per->nombre_pieza) > $max_caracteres){
									echo substr($ref_per->nombre_pieza,0,50).'...';
								}
								else {
									echo $ref_per->nombre_pieza;	
								}
							?>
                            <input type="hidden" name="nombre_pieza_per-<?php echo $ref_per->id_referencia;?>" id="nombre_pieza_per-<?php echo $ref_per->id_referencia;?>" value="<?php echo $ref_per->nombre_pieza;?>" />
						</td>
						<td style="text-align:center">
							<input type="text" id="piezas_per-<?php echo $ref_per->id_referencia;?>" name="piezas_per-<?php echo $ref_per->id_referencia;?>" class="CampoPiezasInput" value="<?php echo $piezas;?>"/>
                        </td>
                        <td style="text-align:center">
							<?php echo number_format($ref_per->pack_precio, 2, '.', ''); ?>
                            <input type="hidden" name="p_precio-<?php echo $ref_per->id_referencia;?>" id="p_precio-<?php echo $ref_per->id_referencia;?>" value="<?php echo number_format($ref_per->pack_precio, 2, '.', '');?>" />                            
                        </td>
                        <td style="text-align:center">
							<?php echo $ref_per->cantidad; ?>
