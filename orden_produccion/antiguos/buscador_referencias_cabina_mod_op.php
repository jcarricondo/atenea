<?php 
// Este es el fichero del buscador de referencias de una cabina en la seccion de modificacion de una orden de produccion
...
// Funcion llama a addRow de MuestraCabinaModOP para añadir la referencia del buscador a la tabla
function add_referencia(id_referencia){
	referencia = document.getElementById("referencia_cab-" + id_referencia).value;
	proveedor = document.getElementById("proveedor_cab-" + id_referencia).value;
	ref_proveedor = document.getElementById("ref_proveedor_cab-" + id_referencia).value;
	nombre_pieza = document.getElementById("nombre_pieza_cab-" + id_referencia).value;
	cantidad = document.getElementById("cantidad_cab-" + id_referencia).value;
	piezas = document.getElementById("piezas_cab-" + id_referencia).value;
	...
	if (!error_piezas) {
		...
		fin_enlace = '</a>';
		cadena_identificador = '<input type="hidden" name="REFS_CAB[]" id="REFS_CAB[]" value="' + id_referencia + '" />';
		cadena_unidades_paq =  '<input type="hidden" name="UDS_CAB[]" id="UDS_CAB[]" value="' + cantidad + '"/>';
		...
		window.opener.addRow('mitablaCabina',id_referencia);
		//window.close();
	}
...
<div id="ContenedorCentralReferencias">
	...
	<div id="ContenedorBuscadorReferencias">
		<h4> Buscar la referencia para añadir a la nueva Orden de Produccion </h4>
   		<form name="BuscadorReferencias" id="BuscadorReferencias" action="buscador_referencias_cabina_mod_op.php" method="post">
				...
				for($i=0;$i<count($resultadosBusqueda);$i++) {
					...
					<tr>
						...
						<td>
							...
							?>
                            <input type="hidden" name="referencia_cab-<?php echo $ref->id_referencia;?>" id="referencia_cab-<?php echo $ref->id_referencia;?>" value="<?php echo $ref->referencia;?>" />
						</td>
						<td>
							<?php echo $ref->proveedor; ?>
                            <input type="hidden" name="proveedor_cab-<?php echo $ref->id_referencia;?>" id="proveedor_cab-<?php echo $ref->id_referencia;?>" value="<?php echo $ref->proveedor;?>" />
						</td>
                        <td>
                        	<?php $ref->vincularReferenciaProveedor();?>
                            <input type="hidden" name="ref_proveedor_cab-<?php echo $ref->id_referencia;?>" id="ref_proveedor_cab-<?php echo $ref->id_referencia;?>" value="<?php echo $ref->ref_proveedor;?>" />
                        </td>
						<td>
							<?php 
							...
							?>
                            <input type="hidden" name="nombre_pieza_cab-<?php echo $ref->id_referencia;?>" id="nombre_pieza_cab-<?php echo $ref->id_referencia;?>" value="<?php echo $ref->nombre_pieza;?>" />
						</td>
						<td style="text-align:center">
							<input type="text" id="piezas_cab-<?php echo $ref->id_referencia;?>" name="piezas_cab-<?php echo $ref->id_referencia;?>" class="CampoPiezasInput" value="<?php echo $piezas;?>"/>
                        </td>
                        <td style="text-align:center">
                        ...
                        <td style="text-align:center">
                            ...
                            <input type="hidden" name="cantidad_cab-<?php echo $ref->id_referencia;?>" id="cantidad_cab-<?php echo $ref->id_referencia;?>" value="<?php echo $ref->cantidad;?>" />                            
                        </td>
                        <td style="text-align:center">
                            <form name="BuscadorReferenciaLibre" id="BuscadorReferenciaLibre" action="muestra_referencias_cabina_mod_op.php?nombreref=<?php echo $ref->referencia;?>&id=<?php echo $ref->id_referencia;?>" method="post">                         <input type="button" onclick="javascript:add_referencia(<?php echo $ref->id_referencia;?>);" value="+" />
