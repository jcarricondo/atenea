<?php 
// Este es el fichero del buscador de referencias libres en la seccion de modificacion de una orden de produccion
...
function add_referenciaLibre(id_referencia){
	...
	if (!error_piezas) {
		...
		fin_enlace = '</a>';
		cadena_identificador = '<input type="hidden" name="REFS_LIBRES[]" id="REFS_LIBRES[]" value="' + id_referencia + '" />';
		cadena_unidades_paq =  '<input type="hidden" name="UDS_REF_LIBRES[] id="UDS_REF_LIBRES[] value="' + cantidad + '"/>';
		identificador = id_referencia;
		boton = '<input type="button" id="menos" name="menos" value="-" onclick="javascript:removeRowRefLibre()"  />'
		...
		window.opener.addRowRefLibre('mitablaRefLibres',id_referencia);
		//window.close();
	}
